...
| Risk-appetite consent gate: vetted template + validated caution (Jul 28) | Higher-risk lanes require logged, informed consent. The bot offers blended tiers (Keep it safe / Grow it carefully / Grow it boldly) only for savings-like buckets; everything else stays conservative by design, enforcing the tax-escrow liquidity rule at the conversation layer. Cautions are Sonnet-generated for warmth but validated against non-negotiable required points from a vetted template (aggressive MUST name impermanent loss, possible principal loss, no guaranteed returns); the validator matches natural phrasings without lowering the bar, and falls back to complete pre-written prose if generation fails. The exact caution shown is logged verbatim to risk_consents as the audit trail. |
| Aggressive lane executed through DCW, end to end (Jul 28) | The full USDC→WETH→Uniswap V3 LP lane runs entirely through Circle developer-controlled wallet contract executions — no local key. Open (approve/swap/approve/mint, incl. struct args with signed int24 ticks) and close (decrease/collect/burn) both proven with real funds, capital returned whole. Wrapped as callable product functions in `lib/executor/aggressive-lane.ts`. Lessons baked in: check-first generous approvals (exact approvals under-fund later steps and revert the mint), estimate-before-execute on struct calls, sequential confirmation. |
| Onboarding→execution loop closed via deployBucket (Jul 28) | `lib/executor/deploy-bucket.ts` reads an obligation, confirms risk_tier=aggressive, bridges USDC from its Arc bucket wallet to the Base DCW wallet via CCTP, waits for arrival, then deploys to the aggressive lane. First chaining of the bridge (Arc→Base) and lane execution, each previously proven alone. Verified: Savings set aggressive, 5 USDC bridged + deployed + exited clean. v1 uses explicit amount + manual trigger; autonomous thresholding and the auto/batched trigger layer on top of this callable core. |
| Autonomous deployment via background schedule (Jul 28) | Auto-deploy is a background process, NOT in the webhook — the payment pipeline stays fast and the cross-chain sequence can't block or time it out. A Netlify scheduled function (every 15 min) hits a secret-protected endpoint that runs the deployer: for each aggressive bucket, if idle balance (on-chain Arc USDC minus pending+active deployments) ≥ 100 USDC, insert a `pending` guard row BEFORE bridging, then bridge+deploy, then flip to `active` with the tokenId. Idempotent by construction — pending counts as deployed, so overlapping or repeat runs can't double-deploy; failures mark `failed` (not counted deployed) so a later run retries. lane_deployments is the source of truth for deployed-vs-idle. Verified: auto-detected 246.95 idle on Savings, deployed to position 81529; immediate re-run skipped (idle 0 < 100). |
| Fiat off-ramp: Crossmint, integrated against live Orders API, production-gated (Jul 28) | The car-payment fiat leg (USDC on Base -> USD to bank) is integrated via Crossmint's Orders API in lib/executor/offramp-bucket.ts, reusing the DCW transfer primitive for the on-chain send. Verified live against staging: authenticates, clears the chain gate (staging accepts base-sepolia; mainnet base is production-only), and reaches Crossmint's order-schema validation. The exact offramp order shape + the withdrawal itself are production-only (offramp isn't available on testnets) — VERIFY-marked and guarded PRODUCTION_PENDING rather than guessed. Coinbase Offramp was evaluated first and dropped (required every user to hold a Coinbase account — a dependency, not a product). Crossmint chosen: agent-native offramp, Base support, pays to any bank via ACH/RTP. Production onboarding requested (sales email sent). This replaces the OFFRAMP_DESTINATION_PENDING_PARTNER_DECISION placeholder once live. |
